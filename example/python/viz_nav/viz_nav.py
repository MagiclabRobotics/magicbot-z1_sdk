...
# -*- coding: utf-8 -*-
"""
MagicBot Z1 navigation GUI (magicbot_z1_sdk).

- Motion: gait, tricks, joystick
...
# Prefer SDK build output next to magicbot_z1_sdk/
...
class AppTheme:
    ...
    def info_box(
        ...
        outer = tk.Frame(parent, bg=self.SURFACE_ALT)
        outer.pack(fill=tk.X, padx=self.PAD_INNER, pady=(1, self.PAD_TAB))
        ...
        lbl.pack(fill=tk.X, padx=self.PAD_INNER + 2, pady=self.PAD_INNER)
        return lbl
    ...
    def listbox_panel(self, parent: tk.Widget, *, height: int) -> Tuple[tk.Frame, tk.Listbox]:
        ...
        body.pack(fill=tk.BOTH, expand=True, padx=self.PAD_INNER, pady=self.PAD_INNER)
...
GAIT_CHOICES: List[ChoiceRow] = [
    (
        "Recovery stand · 恢复站立",
        magicbot.GaitMode.GAIT_RECOVERY_STAND,
        "站立恢复（GAIT_RECOVERY_STAND）。上电后常用初始步态。",
    ),
    (
        "Balance stand · 力控站立",
        magicbot.GaitMode.GAIT_BALANCE_STAND,
        "力控平衡站立，可小幅移动；建图前可先切到此步态。",
    ),
    (
        "Humanoid walk · 人形行走",
        magicbot.GaitMode.GAIT_HUMANOID_WALK,
        "人形行走步态。建图、定位、导航遥控时常用。",
    ),
    (
        "Passive · 卸力",
        magicbot.GaitMode.GAIT_PASSIVE,
        "被动/卸力模式。维护或搬运时使用。",
    ),
    (
        "Hybrid SDK · 混合控制",
        magicbot.GaitMode.GAIT_HYBRID_SDK,
        "混合 SDK 步态（上半身 + 下肢高层）。",
    ),
]

TRICK_CHOICES: List[ChoiceRow] = [
    ("— · 无", magicbot.TrickAction.ACTION_NONE, "不执行动作。"),
    ("Welcome · 欢迎", magicbot.TrickAction.ACTION_WELCOME, "欢迎动作。"),
    ("Shake head · 摇头", magicbot.TrickAction.ACTION_SHAKE_HEAD, "摇头。"),
    ("Left greeting · 左手问候", magicbot.TrickAction.ACTION_LEFT_GREETING, "左手问候。"),
    ("Right greeting · 右手问候", magicbot.TrickAction.ACTION_RIGHT_GREETING, "右手问候。"),
    (
        "Shake left hand · 左手握手",
        magicbot.TrickAction.ACTION_SHAKE_LEFT_HAND_REACHOUT,
        "左手握手（伸出）。",
    ),
    (
        "Shake right hand · 右手握手",
        magicbot.TrickAction.ACTION_SHAKE_RIGHT_HAND_REACHOUT,
        "右手握手（伸出）。",
    ),
    ("Clap hand · 鼓掌", magicbot.TrickAction.ACTION_CLAP_HAND, "鼓掌。"),
    ("Superman wave · 超人挥手", magicbot.TrickAction.ACTION_SUPERMAN_WAVE, "超人挥手。"),
    ("Fly kiss left · 左手飞吻", magicbot.TrickAction.ACTION_FLY_KISS_LEFT, "左手飞吻。"),
    ("Fly kiss right · 右手飞吻", magicbot.TrickAction.ACTION_FLY_KISS_RIGHT, "右手飞吻。"),
    ("Hug reachout · 拥抱伸出", magicbot.TrickAction.ACTION_HUG_REACHOUT, "拥抱（伸出）。"),
    ("Right hand salute · 右手敬礼", magicbot.TrickAction.ACTION_RIGHT_HAND_SALUTE, "右手敬礼。"),
    ("Turn wave left · 左转挥手", magicbot.TrickAction.ACTION_TRUN_WAVE_LEFT, "左转挥手（左）。"),
    ("Turn wave right · 右转挥手", magicbot.TrickAction.ACTION_TRUN_WAVE_RIGHT, "右转挥手（右）。"),
]

# GaitMode numeric values from magicbot_z1_sdk magic_type.h
...
def _lookup_error_code(error_code: int) -> str:
    ...
    if hasattr(magicbot, "lookup_error_code"):
        return magicbot.lookup_error_code(int(error_code))
...
def _gait_fallback_enum(sentinel_val: Optional[int], last_gait: Optional[object]):
    ...
    if sentinel_val == 9999:
        return magicbot.GaitMode.GAIT_RECOVERY_STAND
    return None
...
class VirtualJoystick(ttk.Frame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if label:
            ttk.Label(self, text=label, style="Stick.TLabel", wraplength=size + 56).pack(
                pady=(0, 8)
            )
...
class RobotSession:
    """Thread-safe wrapper around MagicRobot (motion + SLAM + audio)."""
    ...
    def connect(self, local_ip: str, robot_ip: str = "") -> Tuple[bool, str]:
        del robot_ip  # Z1 SDK uses onboard gRPC target; robot_ip is for RTSP only.
        with self._lock:
            if self.connected:
                return True, "Already connected"
            notes: List[str] = []
            self.robot = magicbot.MagicRobot()
            if not self.robot.initialize(local_ip.strip()):
                self.robot.shutdown()
                self.robot = None
            ...
            st = self.robot.set_motion_control_level(magicbot.ControllerLevel.HighLevel)
            if not _status_ok(st):
                self.robot.shutdown()
                self.robot = None
            ...
            if not self.audio.initialize():
                notes.append("audio init failed")
            if not self.high.initialize():
                notes.append("motion init failed")
            if not self.slam.initialize():
                notes.append("slam init failed")
            if not self.monitor.initialize():
                notes.append("monitor init failed")

            self.display = None
            notes.append(f"model={magicbot.get_robot_model()}")
            self.connected = True
            self._start_poll()
            ...
            try:
                if self.slam:
                    ...
                    if _status_ok(st2):
                        with self._lock:
                            self.nav_status_msg = (
                                f"id={nav.id} status={nav.status} "
                                f"err={nav.error_code} {nav.error_desc}"
                            )
                if self.monitor:
                    st3, rs = self.monitor.get_current_state()
                    if _status_ok(st3):
                        with self._lock:
                            self.robot_state = rs
            except Exception as exc:
    ...
    def execute_trick(self, trick) -> Tuple[bool, str]:
        if trick == magicbot.TrickAction.ACTION_NONE:
    ...
    def get_gait(self) -> Tuple[bool, Optional[object], str]:
        ...
        def _do():
            ...
            ok = _status_ok(st)
            if ok and not _gait_is_sentinel(g):
    ...
    def fetch_map_path(self, map_name: str) -> Tuple[bool, str]:
        if not self.slam or not map_name:
            return False, "map name required"
        st, paths = self.slam.get_map_path(map_name, 10000)
        if not _status_ok(st):
            return False, st.message
        if not paths:
            return False, "no map path returned"
        with self._lock:
            self.map_path = paths[0]
    ...
    def get_voice_config_text(self) -> Tuple[bool, str]:
        return False, "Z1 SDK 未提供 get_voice_config 接口"

    def switch_tts_model(self, tts_type) -> Tuple[bool, str]:
        del tts_type
        return False, "Z1 SDK 未提供 switch_tts_voice_model 接口"

    def get_all_face_expressions(self) -> Tuple[bool, List[object], str]:
        return False, [], "Z1 SDK 未提供 Display 表情接口"
    ...
    def get_current_face_expression(self) -> Tuple[bool, Optional[object], str]:
        return False, None, "Z1 SDK 未提供 Display 表情接口"
...
class RtspPlayer:
    ...
    def _capture_loop(self) -> None:
        ...
        if not cap.isOpened():
            ...
            self._log(f"RTSP: cannot open {self._url}")
            self._running = False
            return
    ...
    def _show_frame(self, photo: ImageTk.PhotoImage) -> None:
        if not self._running and self._status != "Playing":
            return
        self._photo = photo
...
class NavVizApp:
    def __init__(self, default_local_ip: str, default_robot_ip: str, rtsp_port: int = 8082) -> None:
        self.rtsp_port = rtsp_port
        self.default_robot_ip = default_robot_ip
        self.session = RobotSession()
        self.root = tk.Tk()
        self.root.title("MagicBot Z1 · Nav Viz")
        ...
        self._main_paned.add(tab_host, minsize=360)
        ...
        self.notebook = ttk.Notebook(nb_shell)
        ...
        self._main_paned.add(log_host, minsize=120)
    ...
    def _build_header(self, parent: ttk.Frame) -> None:
        ...
        tk.Frame(shell, bg=t.HEADER_BAR, height=3).pack(fill=tk.X)
        row = tk.Frame(shell, bg=t.SURFACE)
        row.pack(fill=tk.X, padx=t.PAD_FRAME, pady=(t.PAD_INNER, t.PAD_INNER))
        tk.Label(
            row,
            text="MagicBot Z1",
            bg=t.SURFACE,
        ...
        tk.Label(
            row,
            text="  gRPC  ·  Motion  ·  SLAM  ·  Nav  ·  Audio  ·  Video",
            bg=t.SURFACE,
            fg=t.TEXT_HINT,
    ...
    def _build_connection_bar(self, parent: ttk.Frame, local_ip: str, robot_ip: str) -> None:
        ...
        inner.pack(fill=tk.X, padx=t.PAD_FRAME, pady=t.PAD_INNER)
        ...
        ttk.Label(row, text="Robot IP (RTSP)", style="CardMuted.TLabel").pack(side=tk.LEFT)
        ...
        sep = tk.Frame(inner, bg=t.BORDER_LIGHT, height=1)
        sep.pack(fill=tk.X, pady=(t.PAD_INNER, t.PAD_ROW))
        ...
        ttk.Label(
            status_row,
            text="magicbot_z1_sdk · HighLevel + SLAM",
    ...
    def _init_main_paned_sash(self) -> None:
        ...
        try:
            ...
            if h > 180:
                self._main_paned.sashpos(0, max(320, h - 140))
        except tk.TclError:
            pass
    ...
    def _build_log_panel(self, parent: ttk.Frame) -> None:
        ...
        fb_row = ttk.Frame(lf, style="Card.TFrame")
        fb_row.pack(fill=tk.X, padx=self.theme.PAD_INNER, pady=(self.theme.PAD_INNER, 2))
    ...
    def _build_motion_tab(self) -> None:
        ...
        gait_row.pack(fill=tk.X, padx=t.PAD_INNER, pady=(t.PAD_INNER, t.PAD_ROW))
        ...
        gait_status_row.pack(fill=tk.X, padx=t.PAD_INNER, pady=(0, t.PAD_ROW))
        ...
        trick_row.pack(fill=tk.X, padx=t.PAD_INNER, pady=(t.PAD_INNER, t.PAD_ROW))
        ...
        self._joy_frame.pack(fill=tk.BOTH, expand=True, padx=t.PAD_FRAME, pady=(t.PAD_ROW, t.PAD_TAB))

        joy_row = ttk.Frame(self._joy_frame, style="Card.TFrame")
        joy_row.pack(fill=tk.BOTH, expand=True, padx=t.PAD_INNER, pady=t.PAD_INNER)
        ...
        self.left_stick.pack(side=tk.LEFT, padx=20)
        ...
        self.right_stick.pack(side=tk.LEFT, padx=20)
        ...
        state_inner.pack(fill=tk.BOTH, expand=True, padx=t.PAD_INNER, pady=t.PAD_INNER)
        ...
        bf.pack(fill=tk.X, pady=t.PAD_ROW, padx=t.PAD_INNER)
    ...
    def _scrollable_frame(self, parent: tk.Widget) -> ttk.Frame:
        ...
        viewport.pack(fill=tk.BOTH, expand=True, padx=t.PAD_INNER, pady=t.PAD_INNER)
    ...
    def _slam_flow_arrow(self, parent: ttk.Frame, text: str) -> None:
        ttk.Label(parent, text=text, style="FlowHint.TLabel").pack(
            anchor=tk.CENTER, pady=self.theme.PAD_ROW
        )
    ...
    def _slam_step(
        ...
        row.pack(fill=tk.X, padx=self.theme.PAD_INNER, pady=self.theme.PAD_ROW)
        ...
        badge_lbl = tk.Label(
            ...
            font=self.theme.font_ui_bold,
            padx=6,
            pady=2,
        )
        badge_lbl.grid(row=0, column=0, rowspan=2, padx=(0, 8), sticky=tk.N)
        ttk.Label(row, text=title, style="StepTitle.TLabel").grid(row=0, column=1, sticky=tk.W)
        ttk.Label(row, text=detail, style="StepDetail.TLabel", wraplength=480).grid(
            row=1, column=1, sticky=tk.W, pady=(1, 0)
        )
    ...
    def _build_slam_tab(self) -> None:
        ...
        self.theme.info_box(
            ...
            "建图/导航前请在 Motion 页将步态切为 Humanoid walk 或 Balance stand。"
        ...
        list_panel, self.map_list = self.theme.listbox_panel(phase_b, height=6)
        ...
        ttk.Label(
            ...
            style="CardMuted.TLabel",
            wraplength=t.INFO_WRAP,
        ).pack(anchor=tk.W, padx=4, pady=(t.PAD_INNER, 0))
    ...
    def _build_nav_map_tab(self) -> None:
        ...
        ttk.Label(inner, text="导航目标", style="Card.TLabel").grid(row=1, column=0, sticky=tk.W, pady=(8, 2))
        ...
        self.fig = Figure(figsize=(5.6, 5.2), dpi=96, facecolor=self.theme.SURFACE)
    ...
    def _default_rtsp_url(self) -> str:
        return f"rtsp://{self._robot_host()}:{self.rtsp_port}"
    ...
    def _build_audio_tab(self) -> None:
        ...
        self.theme.info_box(
            scroll,
            "音量与 TTS 通过 magicbot_z1_sdk AudioController（gRPC）与机载服务通信。",
        )
        ...
        self.tts_content.insert("1.0", "你好，我是 MagicBot Z1。")
        ...
        cfgf = ttk.LabelFrame(scroll, text=" 语音配置（Z1 暂未开放） ")
    ...
    def _tts_priority_value(self):
        m = {
            "HIGH": magicbot.TtsPriority.HIGH,
            "MIDDLE": magicbot.TtsPriority.MIDDLE,
            "LOW": magicbot.TtsPriority.LOW,
        }
        return m.get(self.tts_priority_var.get(), magicbot.TtsPriority.HIGH)

    def _tts_mode_value(self):
        m = {
            "CLEARTOP": magicbot.TtsMode.CLEARTOP,
            "ADD": magicbot.TtsMode.ADD,
            "CLEARBUFFER": magicbot.TtsMode.CLEARBUFFER,
        }
        return m.get(self.tts_mode_var.get(), magicbot.TtsMode.CLEARTOP)

    def _tts_type_value(self):
        m = {
            "NONE": magicbot.TtsType.NONE,
            "DOUBAO": magicbot.TtsType.DOUBAO,
            "GOOGLE": magicbot.TtsType.GOOGLE,
        }
        return m.get(self.tts_type_var.get(), magicbot.TtsType.DOUBAO)
    ...
    def _build_video_tab(self) -> None:
        ...
        self.theme.info_box(
            ...
            "边看画面边遥控；摇杆与 Motion 页共用一条 20Hz 推流。",
        )
        t = self.theme
        ...
        paned.add(preview_wrap, minsize=400)
        paned.add(joy_wrap, minsize=260)
        ...
        self.rtsp_player = RtspPlayer(
            self.root, self.rtsp_label, self._log, on_stats=self._on_rtsp_stats
        )
    ...
    def _build_video_joy_panel(self, parent: ttk.Frame) -> None:
        ...
        bf = ttk.Frame(self._video_joy_frame, style="Card.TFrame")
        ...
        status_row = ttk.Frame(self._video_joy_frame, style="Card.TFrame")
        ...
        ttk.Label(
            ...
            style="Hint.TLabel",
            wraplength=280,
        ).pack(padx=t.PAD_INNER, pady=(0, t.PAD_TAB))
    ...
    def _on_rtsp_start(self) -> None:
        if not hasattr(self, "rtsp_player"):
            ...
            return
        if self.rtsp_player.start(self.rtsp_url_var.get()):
            self._update_rtsp_stream_ui()
            self._schedule_rtsp_status()
            self._log_action("RTSP Start", True, self.rtsp_url_var.get())
        else:
            self._log_action("RTSP Start", False, "无法打开流")
    ...
    def _schedule_rtsp_status(self) -> None:
        ...
        if not hasattr(self, "rtsp_player"):
            return
        st = self.rtsp_player.status
        if self.rtsp_player.is_active or st not in ("Stopped",):
    ...
    def _update_rtsp_stream_ui(self) -> None:
        ...
        if player.is_active and st == "Playing":
            ...
            self.rtsp_status_label.config(text=st, style="StatusBad.TLabel")
    ...
    def _selected_map_name(self) -> str:
        ...
        if sel:
            return self.map_list.get(sel[0]).strip()
        return (self.session.current_map_name or "").strip()
    ...
    def _on_start_mapping(self) -> None:
        def _start():
            st = self.session.slam.activate_slam_mode(magicbot.SlamMode.MAPPING, "", 10000)
            if not _status_ok(st):
                return st
            return self.session.slam.start_mapping(10000)

        self._slam_cmd(_start, "start_mapping")
    ...
    def _on_switch_loc(self) -> None:
        ...
        if not name:
            self._log_action("切换定位", False, "请先在列表中选择或加载地图")
            return
        ok, msg = self.session.fetch_map_path(name)
        if not ok:
            self._log_action("切换定位", False, msg)
            return
        self._slam_cmd(
            lambda: self.session.slam.activate_slam_mode(
                magicbot.SlamMode.LOCALIZATION, self.session.map_path, 10000
            ),
    ...
    def _on_slam_idle(self) -> None:
        self._slam_cmd(
            lambda: self.session.slam.activate_slam_mode(magicbot.SlamMode.IDLE, "", 10000),
            "activate_slam_idle",
        )
    ...
    def _on_load_map(self) -> None:
        ...
        def _load():
            st = self.session.slam.load_map(name, 10000)
            if _status_ok(st):
                self.session.fetch_map_path(name)
            return st

        self._slam_cmd(_load, "load_map")
        self.root.after(800, self._on_refresh_maps)
    ...
    def _on_nav_on(self) -> None:
        if not self.session.map_path:
            name = self._selected_map_name() or self.session.current_map_name
            if name:
                ok, msg = self.session.fetch_map_path(name)
                if not ok:
                    self._log_action("开启导航", False, msg)
                    return
            else:
                self._log_action("开启导航", False, "请先加载地图并获取 map path")
                return

        self._slam_cmd(
            lambda: self.session.slam.activate_nav_mode(
                magicbot.NavMode.GRID_MAP, self.session.map_path, 10000
            ),
            "activate_nav_mode",
        )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="MagicBot Z1 navigation GUI (magicbot_z1_sdk)")
    parser.add_argument("--local-ip", default="192.168.54.111", help="PC IP on robot network")
    parser.add_argument("--robot-ip", default="192.168.54.119", help="Robot IP for RTSP preview")
    parser.add_argument("--rtsp-port", type=int, default=8082, help="RTSP port on robot")
    args = parser.parse_args()
    app = NavVizApp(args.local_ip, args.robot_ip, rtsp_port=args.rtsp_port)
    app.run()
